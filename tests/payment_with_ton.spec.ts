...
describe('pay_with_ton', () => {
    ...
    it('should transfer ton', async () => {
        ...
        console.log('bobWalletData before transfer:', fromNano(bobWalletData.balance.toString()));

        // 确保 toAddress 是 bobWallet.address
        await adminWallet.sendTransfer(admin.getSender(), {
            toAddress: bobWallet.address, // 修改为 bobWallet.address
            value: toNano('0.5'),
            jettonAmount: toNano('2'),
            fwdAmount: toNano('0.2'),
            fwdPayload: beginCell().endCell(),
            responseAddress: bobWallet.address, // 确保响应地址也是 bobWallet.address
        });

        const bobWalletData_2 = await bobWallet.getWalletData();
        console.log('bobWalletData after transfer:', fromNano(bobWalletData_2.balance.toString()));

        expect(bobWalletData_2.balance).toBe(toNano('2')); // 确保余额正确
    });
